...
export class DictionaryDetailService {
  ...
  initMenu() {
    ...
    this.manualMenu = this.dictArr.filter(el => el.dictCode === PRODUCT_PARENT_CODE).reduce((arr: NzTreeNodeOptions[], el) => {
      arr.push({
        title: el.itemName,
        key: el.itemCode,
        children: ProductChild.filter(child => child.pitemCode === el.itemCode).reduce((childArr: NzTreeNodeOptions[], childNode) => {
          childArr.push({
